...
import {
  NoProducts,
  PointsAlert,
} from "@/components/adminPanel/catalogue/NoProducts";
...
const CatalogueIndex = ({ products, user }) => {
  ...
  const itemsPerPage = () => {
    if (!filterPoints) {
      if (search.length === 0 && currentPage === 0)
        return products?.result.slice(currentPage, currentPage + 6);
      let filteredResults = products?.result.filter((r) =>
  ...
  const nextPage = () => {
    if (
      products?.result.filter((c) => c.productDescription.includes(search))
        .length >
      currentPage + 6
    ) {
  ...
  return (
    <>
      <Head>
        <title>Primates - Product catalogue</title>
      </Head>
      <Layout>
        <Grid
          container
          ...
          sx={{ mb: 1, mt: 1 }}
        >
          <Typography variant="h4">Primates Store</Typography>
          ...
          >
            ...
            <Button
              ...
              sx={{
                width: "20%",
                backgroundColor: "#ddd",
                ":hover": { color: "white", backgroundColor: "#FB923C" },
              }}
              onClick={() => setFilterPoints(!filterPoints)}
            >
            ...
            {itemsPerPage()?.length === 0 ? (
              ...
              <>
                ...
                >
                  ...
                  <Button
                    onClick={nextPage}
                    disabled={currentPage + 6 >= products?.result.length}
                  >
                    <ArrowForwardIosIcon
                      fontSize="large"
                      color="primary.main"
...
export const getServerSideProps = async (context) => {
  try {
    ...
    const res = await axios.get(
      `https://localhost:7149/api/Catalogue?page=1&pageSize=100`,
      {
        headers: {
