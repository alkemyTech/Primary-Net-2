...
const UpdateRoleForm = ({ role }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { data, status } = await axios.put(`https://localhost:7149/api/Role/${role?.id}`,
                {
                    name,
                    description
                },
    ...
    return (
        <>
        <Head>
            <title>Primates - Admin Role update</title>
        </Head>
            {
                showConfirm && <ConfirmSweetAlert
                    title='Confirmation'
                    text={`Are you sure you want to update the role ${role.id}?`}
...
export const getServerSideProps = async (context) => {
    ...
    try {
        ...
        const { data } = await axios.get(`https://localhost:7149/api/Role/${id}`, {
            headers: {
                'Authorization': `Bearer ${session.user?.token}`,
                "Content-Type": "application/json",
            }
