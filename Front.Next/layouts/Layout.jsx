...
export const Layout = ({ children }) => {
  ...
  }, [session?.user?.accountId, session?.user?.token, setIsAccountLocked]);
  ...
  return (
    <>
      ...
      {showAlertLock && (
        <ConfirmSweetAlert
          title={`${isAccountLocked ? "Unlock" : "Lock"} account`}
          ...
          onClose={() => setShowAlertLock(false)}
          onCancel={() => setShowAlertLock(false)}
        />
      )}
      ...
      {unlockedAlert && (
        <SweetAlert
          ...
          icon="success"
          timer={4000}
          onClose={() => setUnlockedAlert(false)}
        />
      )}
      <Grid container>
        <Navbar
          ...
          setShowAlertLock={setShowAlertLock}
        />
